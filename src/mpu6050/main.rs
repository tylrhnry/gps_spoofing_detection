...
fn main() {
  ...
  loop {
    ...
    if start.elapsed() >= Duration::from_secs(3) {
      println!("Iterations in 3 sec.: {iters}");
      break;
    };

    let accel_point @ DataPoint {x: a_x, y: a_y, z: a_z} = get_acceleration(&mut i2c);
    accel_vec.push(accel_point);

    let gyro_point @ DataPoint {x: g_x, y: g_y, z: g_z} = get_gyroscope(&mut i2c);
    gyro_vec.push(gyro_point);


    println!("Accelerometer: X={},\tY={},\tZ={}", a_x, a_y, a_z);
    println!("Gyroscope:     X={},\tY={},\tZ={}", g_x, g_y, g_z);
    iters += 1;
  }
